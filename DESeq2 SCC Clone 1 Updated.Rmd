...
cts <- read_tsv('/Users/elenajochum/Desktop/Research/Izumchenko Lab/Honors Thesis/SCC Clone 1/count_scc_human_AS.tsv')
cts<- as.data.frame(cts)
rownames(cts) <- cts[,1]
cts <- cts[,-1]
coldata <- read.csv('/Users/elenajochum/Desktop/Research/Izumchenko Lab/Honors Thesis/SCC Clone 1/Metadata_scc_human.csv', row.names=1)
```
...
```{r}
# REMOVE TWO SAMPLES THAT DO NOT CLUSTER IN PCA PLOT
# Remove L24
coldata <- coldata[-3,]
cts <- cts[,-3]
...
```{r}
# Construct DESeqDataSet without outliers
coldata$Condition <- factor(coldata$Condition)
dds <- DESeqDataSetFromMatrix(countData = cts,
                              colData = coldata,
                              design = ~ Condition)
...
res <- results(dds)
res_clone1vsControl <- results(dds, name="Condition_clone1_vs_Control", alpha=0.05) # alpha is p-value

# Change ENSG IDs to gene symbol
res_clone1vsControl$symbol <- mapIds(org.Hs.eg.db,keys=row.names(res_clone1vsControl),column="SYMBOL", keytype="SYMBOL", multiVals="first")
res_clone1vsControl <- as.data.frame(res_clone1vsControl)
res_clone1vsControl <- res_clone1vsControl[,c(7, 1, 2, 3, 4, 5, 6)]
write.csv(as.data.frame(res_clone1vsControl), file="results_clone1_vs_control.csv") # write results to csv
```

```{r}
# PCA plot with outliers removed
vsd <- vst(dds, blind=FALSE)
rld <- rlog(dds, blind=FALSE)
head(assay(vsd), 3)
pca <- plotPCA(vsd, intgroup=c("Condition"))
nudge <- position_nudge(y = 1, x = -2)
pca + geom_label(aes(label = name), position = nudge)
```

```{r}
# Look at differential expression analysis results by padj and LFC
res <- res_clone1vsControl # change this depending on which comparison we want to look at from above
...
pheatmap(sc_1, cellheight=10, cellwidth = 10, filename = 'SCC_Clone1_vs_Ctrl_up_heatmap.png')
```
...
pheatmap(sc_1_down, cellheight=10, cellwidth = 10, filename = 'SCC_Clone1_vs_Ctrl_down_heatmap.png')
```

```{r}
# Heatmap with significant genes of interest
degs <- rbind(upregulated, downregulated)
write_csv(degs, '/Users/elenajochum/Desktop/Research/Izumchenko Lab/Honors Thesis/SCC Clone 1/SCC_Clone1_degs.csv')
...
pheatmap(sc_1_sel, cellheight=10, cellwidth = 10, filename = 'SCC_Clone1_vs_Ctrl_heatmap.png')
```
...
res_up_go
pdf("GO_up_clone1_vs_control.pdf")
dotplot(res_up_go,showCategory=20)
write.csv(as.data.frame(res_up_go),file = "Clone1vsControl _GO_upregulated.csv",row.names = F,quote = F,sep = ',')
```
...
res_down_go
pdf("GO_down_clone1_vs_control.pdf")
dotplot(res_down_go,showCategory=20)
write.csv(as.data.frame(res_down_go),file = "Clone1vsControl _GO_downregulated.csv",row.names = F,quote = F,sep = ',')
```